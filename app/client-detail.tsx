...
export default function ClientDetail() {
  ...
  const [editForm, setEditForm] = useState({ name: '', email: '', phone: '', company: '', address: '', notes: '' });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setEditForm({ name: clientRes.data.name, email: clientRes.data.email || '', phone: clientRes.data.phone || '', company: clientRes.data.company || '', address: clientRes.data.address || '', notes: clientRes.data.notes || '' });
  ...
  const handleSave = async () => {
    if (!id || !editForm.name.trim()) { Alert.alert(t('error'), t('enterAmount')); return; }
    try {
  ...
  const handleDelete = () => {
    Alert.alert(t('deleteClient'), t('confirmDeleteClient'), [
      ...
      { text: t('delete'), style: 'destructive', onPress: async () => {
        try { await clientsApi.delete(id!); removeClient(id!); router.back(); }
        catch (error) { Alert.alert(t('error'), t('failedToDelete')); }
      }},
    ]);
  };
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}
        ...
        <View style={[styles.clientCard, { backgroundColor: theme.card }]}>
          ...
          {editing ? (
            <TextInput style={[styles.editCompanyInput, { backgroundColor: theme.backgroundLight, color: theme.textSecondary }]} value={editForm.company} onChangeText={(t2) => setEditForm({ ...editForm, company: t2 })} placeholder={t('company')} placeholderTextColor={theme.textMuted} />
          ) : client.company ? <Text style={[styles.clientCompany, { color: theme.textSecondary }]}>{client.company}</Text> : null}
        </View>
