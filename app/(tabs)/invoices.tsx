...
export default function InvoicesScreen() {
  ...
  const handleStatusChange = async (invoice: Invoice, newStatus: string) => {
    if (newStatus === 'paid' && invoice.status !== 'paid') {
      Alert.alert(t('registerAsIncome'), `${t('registerAsIncomeQuestion')}\n\n${fmt(invoice.total)}`, [
        ...
        { text: t('registerAsIncome'), onPress: async () => {
          ...
          try {
            ...
            Alert.alert(t('success'), t('incomeRegistered'));
  ...
  const doStatusChange = async (invoice: Invoice, newStatus: string) => {
    try {
      ...
      loadData();
    } catch (e) { Alert.alert(t('error'), t('failedToCreate')); }
  };

  const showStatusOptions = (invoice: Invoice) => {
    const statuses = ['draft', 'sent', 'approved', 'paid', 'overdue'];
    Alert.alert(t('changeStatus'), invoice.invoice_number,
  ...
  const handleDelete = (invoice: Invoice) => {
    Alert.alert(t('deleteInvoice'), t('confirmDeleteInvoice'), [
      { text: t('cancel'), style: 'cancel' },
      { text: t('delete'), style: 'destructive', onPress: async () => {
        try { await invoicesApi.delete(invoice.id); removeInvoice(invoice.id); setSelectedInvoice(null); } catch (e) { Alert.alert(t('error'), t('failedToDelete')); }
      }},
    ]);
  };
  ...
  const exportPDF = async (invoice: Invoice) => {
    ...
    try {
      ...
      try { await Print.printAsync({ html }); } catch (e) { Alert.alert(t('error'), 'PDF export failed'); }
    }
  };
